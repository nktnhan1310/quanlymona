...
namespace QuanLy.Service
{
    public class ProjectTypeService : CatalogueService<ProjectTypes, BaseSearch>, IProjectTypeService
    {
        ...
        public ProjectTypeService(IAppUnitOfWork unitOfWork, IMapper mapper
            , IAppDbContext Context) : base(unitOfWork, mapper)
        {
            this.Context = Context;
            this.IsUseStore = true;
        }
        public override async Task<string> GetExistItemMessage(ProjectTypes item)
        {
            var message = "";
            var isExists = await this.unitOfWork.Repository<ProjectTypes>().GetQueryable()
                .Where(x =>
                x.Id != item.Id &&
                x.Code == item.Code)
                .AnyAsync();
        ...
        {
            return "Mona_sp_Load_Project_Types_PagingData";
        }
    }
}
